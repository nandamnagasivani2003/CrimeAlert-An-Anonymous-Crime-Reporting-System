...
def readDetails(contract_type):
    ...
    deployed_contract_address = '0x0C3f2B47a4F1456ff3bcC646e4A72F3e8dB6aa99'
...
def saveDataBlockChain(currentData, contract_type, username, password, contact, gender, email, address, activity, description, hashcode, timestamp, filename, prediction):
    ...
    deployed_contract_address = '0x0C3f2B47a4F1456ff3bcC646e4A72F3e8dB6aa99'  # contract address
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        # fetch contract's abi - necessary to call its functions
        contract_abi = contract_json['abi']
    ...
    if contract_type == 'signup':
        ...
        signup_detail = SignupDetail(username=username,password=password,contact=contact,gender=gender,email=email,address=address)
        signup_detail.save()
    if contract_type == 'tips':
        details += currentData
...
def SubmitTipAction(request):
    if request.method == 'POST':
        ...
        current_time = timezone.now()  # Use aware datetime object
        ...
        tips_detail = TipsDetail.objects.create(
            username=user, address=address, activity=activity,
            description=desc, hashcode=str(hashcode),
            timestamp=current_time, filename=filename, prediction=predict)
        tips_detail.save()

        data = user + "#" + address + "#" + activity + "#" + desc + "#" + \
            str(hashcode) + "#" + str(current_time) + "#" + filename + "#" + predict + "\n"
        ...
        saveDataBlockChain(data, "tips", username, password, contact, gender, email, address, activity, desc, hashcode, timestamp, filename, prediction)
...
def ViewTip(request):
    if request.method == 'GET':
        ...
        tips_details = TipsDetail.objects.filter(username=request.session.get('username', None))
        readDetails('tips')
        arr = details.split("\n")
        for i in range(len(arr)-1):
            array = arr[i].split("#")
            if array[0] == request.session['username']:
                '''
                ...
                strdata += '<tr><td><font size="" color="black">' + \
                    str(array[0])+'</td><td><font size="" color="black">'+array[1] + \
                ...
                strdata += '<td><img src="/static/files/' + \
                    array[6]+'" height="200" width="200"></img></td></tr>'
...
def RegisterAction(request):
    if request.method == 'POST':
        ...
        if status == "none":
            ...
            context = {
                "data": "Signup process completed and record saved in Blockchain"}
